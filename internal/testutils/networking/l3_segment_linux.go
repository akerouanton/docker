...
const (
	...
	ifnamsiz = 16 // https://elixir.bootlin.com/linux/v6.11/source/include/uapi/linux/if.h#L33
)
...
func NewL3Segment(t *testing.T, nsName string, addrs ...netip.Prefix) *L3Segment {
	...
	for _, addr := range addrs {
		l3.bridge.Run(t, "ip", "addr", "add", addr.String(), "dev", l3.bridge.Iface)
...
func (l3 *L3Segment) AddHost(t *testing.T, hostname, nsName, ifname string, addrs ...netip.Prefix) {
	...
	for _, addr := range addrs {
		host.Run(t, "ip", "addr", "add", addr.String(), "dev", host.Iface)
	}
}
