...
func TestAccessPublishedPortFromAnotherNetwork(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			payload := fmt.Sprintf("foobar%d", i)
			clientID := container.Run(ctx, t, c,
				container.WithName("client"),
				container.WithCmd("/bin/sh", "-c", fmt.Sprintf("echo '%s' | nc -w1 %s 5000", payload, tc.daddr)),
			...
			assert.Assert(t, strings.Contains(logs, payload), "Payload was not received by the server container")
		})
	}
}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.gwMode == "nat" {
				...
				res := sendPayloadFromHost(t, l3.Hosts["attacker"], hostIP, hostPort, "foobar", func() bool {
					logs := getContainerStdout(t, ctx, c, serverID)
					return strings.Contains(logs, "foobar")
				})
			...
			expDirectAccess := tc.gwMode == "routed"
			...
			res := sendPayloadFromHost(t, l3.Hosts["attacker"], ctrIP.String(), hostPort, "bar baz", func() bool {
				logs := getContainerStdout(t, ctx, c, serverID)
				return strings.Contains(logs, "bar baz")
			})
...
func TestAccessPortPublishedOnLoopbackAddress(t *testing.T) {
	...
	res := sendPayloadFromHost(t, l3.Hosts["docker"], loIP, hostPort, "foobar", func() bool {
		logs := getContainerStdout(t, ctx, c, serverID)
		return strings.Contains(logs, "foobar")
	})
	...
	res = sendPayloadFromHost(t, l3.Hosts["attacker"], loIP, hostPort, "bar baz", func() bool {
		logs := getContainerStdout(t, ctx, c, serverID)
		return strings.Contains(logs, "bar baz")
	})
