...
// collectFirewallPorts collects all the types.PortBinding needed to reconfigure
// the host firewall for a given list of port bindings.
func collectFirewallPorts(pbs []portmapperapi.PortBinding) []types.PortBinding {
	var fwdPorts, natPorts []types.PortBinding
	for _, pb := range pbs {
		if pb.Forwarding {
			fwdPorts = append(fwdPorts, toFwdBinding(pb))
		}
		if pb.NAT.IsValid() {
			natPorts = append(natPorts, toNATBinding(pb))
		}
	}
	return append(fwdPorts, natPorts...)
}
