...
func NewNamedNetns(t *testing.T, name string, reusePrevious bool) Netns {
	...
	if err == nil {
		...
		t.Logf("Network namespace %s already exists whereas TEST_REUSE_L2SEGMENT is empty.", newNs.name)
		t.FailNow()
	}
...
func (ns Netns) Set() error {
	...
	if err := netns.Set(ns.handle); err != nil {
		return fmt.Errorf("could not switch to netns %s: %v", ns.String(), err)
	}
	return nil
}
...
// NetlinkHandle returns a *netlink.Handle out of the Netns.
...
// Close closes the fd associated to this Netns. If this is a named Netns, Destroy has to be called too.
...
// handle if it's still open.
func (ns Netns) Destroy(logger Logger) {
...
// routing and iptables rules set on the host.
...
func (sgmt *L2Segment) Destroy(logger Logger) {
...
// named after bridgedIfaceName in BridgeNs. The "host" side receives the addr IP address. It returns the MAC address
// of the "host" side, a function to remove this interface and an error, if any. The veth pair should be ready to use
// once this function returns back.
func (host *L3Host) createVeth(bridgeNlh *netlink.Handle, bridgeIface netlink.Bridge, reusePrevious bool) error {
	...
	if err != nil {
		return fmt.Errorf(
			"could not get host iface %s from netns %s: %w", host.HostIfaceName, host.Ns, err)
	}
	if err := hostNlh.AddrAdd(hostIface, &netlink.Addr{
		...
		return fmt.Errorf(
			"could not add address %s to %s@%s: %w",
			host.IPAddr, bridgedIface.Attrs().Name, host.HostIfaceName, err)
	}
...
func (host *L3Host) Destroy(logger Logger) {
