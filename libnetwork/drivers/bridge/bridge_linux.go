...
var newFirewaller = func(config firewaller.Config) (firewaller.Firewaller, error) {
	if nftables.Enabled() {
		return nftabler.NewNftabler(config)
	}
...
func (d *driver) createNetwork(ctx context.Context, config *networkConfiguration) (err error) {
	...
	bridgeSetup.queueStep("newFirewallerNetwork", func(*networkConfiguration, *bridgeInterface) error {
...
func (d *driver) RevokeExternalConnectivity(nid, eid string) error {
	// Make sure this doesn't function isn't deleting iptables rules while handleFirewalldReloadNw
