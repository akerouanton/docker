...
func (d *driver) CreateEndpoint(nid, eid string, opts driverapi.EndpointOptions) (driverapi.EndpointOptions, error) {
	...
	epOption, err := parseEndpointOptions(opts.DriverOpts)
	if err != nil {
		return driverapi.EndpointOptions{}, err
	}
	epConnectivity, err := ParseEndpointConnectivity(opts.DriverOpts)
	if err != nil {
		return driverapi.EndpointOptions{}, err
	}
	...
	if opts.MACAddress != nil {
		endpointStruct.MacAddress = strings.ReplaceAll(opts.MACAddress.String(), ":", "-")
	}
	...
	if opts.Addr != nil {
		endpointStruct.IPAddress = opts.Addr.IP
	}
